...
**最小闭环**：准备文本 → 词元化 → 构建 Decoder-only 模型 → 下一 token 训练 → 验证 → 生成。第一阶段和第三阶段中的许多方法会让这个闭环更稳、更可解释，但不要求在第一个小模型里一次用完所有技术。Chapter 0 已提供归一化理论与固定数据实验；后续章节会沿用“公式/参数解释 → PyTorch 调用 → 可复现实验 → 如何读结果”的教案格式。
