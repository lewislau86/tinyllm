...
### 10.1 为什么归一化有时能让低精度计算更稳？

把 FP16 想成一把**刻度有限、量程也有限**的尺子。归一化不会增加这把尺子的刻度数；它改变的是送入**后续运算**的数值大小。以 RMSNorm 为例，先把一个向量除以它的均方根，归一化后的均方根约为 1（忽略 `eps` 和可学习的 `weight`）。LayerNorm 还会减均值，使该向量均值约为 0、方差约为 1。这样，后续矩阵乘法、平方或注意力点积遇到极大数值而溢出的机会**可能降低**。这是“控制数值范围”，不是“提高浮点格式的精度”。[PyTorch 数值精度说明](https://docs.pytorch.org/docs/stable/notes/numerical_accuracy.html) 也建议对极端输入考虑归一化或截断，并检查非有限值。

例如，FP16 能存下 `1000`，却存不下 `1000 × 1000`（会溢出）；若先在安全的计算路径中把一组约为 `1000` 的激活归一化到约为 1，再转换成 FP16 去平方，结果就可能保持有限。**顺序很重要：**若在归一化内部已经用低精度算出了 `inf`，或者输入转换时已丢失小差异，事后归一化不能修复。notebook 的第 9 节有“先低精度平方”和“先用 FP32 平方”的对照；这里解释的是为什么把归一化放在后续敏感运算**之前**可能有帮助。

归一化也不是“低精度安全保证”：除以很小的 RMS/标准差可能放大噪声，所以公式需要 `eps`；可学习的缩放参数、后续权重与残差相加又可能把数值放大。BF16 的范围较宽，但刻度较粗；归一化无法恢复 BF16 输入中已被舍入掉的差异。因此要分别观察**激活是否溢出**、**结果离 FP32 参考有多远**和**训练是否收敛**，不能只凭归一化后的范围判断“精度变好了”。完整的浮点格式、混合精度和误差实验放在规划的 Chapter 15。
